...
/*
To-Do
 -background music plays and lanturn returns to white after playing an exerpt (timer)
...
Locations home = new Locations("home", 41.7359, -74.6743, 100, 255, 255, 0, 0, 0, "track1.mp3");
...
void draw() {
  if (input != null) {
    if (input.equals("FLASHLIGHT MODE")) {
      ...
      currLat = data[0];
      currLon = data[1];
      //print("float conversion: " + currLat + "," + currLon);
      if (track[prevLocIndex + 2].isPlaying()) {
        return;
      } else {
        ...
        for ( locIndex = 0; locIndex < waypoint.length; locIndex++) {
          if (inRange( waypoint[locIndex].locLat, waypoint[locIndex].locLon)) {
            ...
            eventTrigger(locIndex + 2); //plus two to correct for bgMusic + noFix in tracks
            bulbUpdate(locIndex);
...
void output(int hue, int sat, int vib, int fadeRate, int pulseRate, int rainbowRate) {
  String comma = ",";
  String finalOutput = str(hue) + comma + str(sat) + comma + str(vib) + comma + str(fadeRate) + comma + str(pulseRate) + comma + str(rainbowRate);
  println(finalOutput);
