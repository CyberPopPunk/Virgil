  ...
  if (startupFinished == true) {
    ...
    while (firstContact == false) {
      ...
      if ( str(input.charAt(0)).equals("A")) {
        ...
        //port.clear();
        println("Input after clear" + input);
        return;
      }
    } 
    if (input != null) {
...
void output(int hue, int sat, int vib, int fadeRate, int pulseRate, int rainbowRate) {
  delay(100);
  String comma = ",";
  String finalOutput = 'A' + str(hue) + comma + str(sat) + comma + str(vib) + comma + str(fadeRate) + comma + str(pulseRate) + comma + str(rainbowRate);
  println(finalOutput);
